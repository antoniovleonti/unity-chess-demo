...
class Pawn : IPiece
{
    ...
    public void MakeMove(Board board, Move move, int player)
    {
        ...
        //
        if (move.src.y == (int)(3.5f + player)
            && board.value[move.src.y, move.dst.x] == -player
            && board.touch[move.src.y, move.dst.x] == 1
            && board.touch[move.src.y + player * 2, move.dst.x] == 1)
        {
            board.value[move.src.y, move.dst.x] = 0;
        }
    }
